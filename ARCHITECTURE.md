...
Native Messaging manifests are browser-specific. Chromium manifests contain only explicitly trusted `chrome-extension://` origins; Firefox manifests contain only explicitly trusted add-on IDs. The deterministic unpacked Chromium ID and Conduit's Firefox ID are defaults. Store-assigned Chromium IDs are added with `conduit extension trust <id>`, which rewrites and verifies the user-level host manifests. Unknown callers are denied before the host reads a request.
