...
The standalone [`conduit-extension`](https://github.com/err0rgod/conduit-extension) repository owns `packages/browser-core` and `apps/extension`. One key-free Chromium store archive serves Chrome, Edge, and Brave; Firefox receives a Gecko-specific archive without Chromium's debugger capability. Browser operations use tabs, scripting, and narrowly requested optional APIs. HTTP and HTTPS host access stays optional, and the popup is the user-controlled boundary for granting or revoking the current origin, explicitly requesting or revoking the exact broad patterns `http://*/*` and `https://*/*`, pausing all control, reviewing pending one-time confirmations, and viewing bounded redacted audit metadata. The broad request runs only from the popup click gesture; background code never requests it, and the browser owns the permission state rather than Conduit storage.

Native Messaging manifests are browser-specific. Chromium manifests contain only explicitly trusted `chrome-extension://` origins; Firefox manifests contain only explicitly trusted add-on IDs. The deterministic unpacked Chromium ID, the published Chrome Web Store ID `gjhipjgiapijcdnflldnoenafeegmfpc`, and Conduit's Firefox ID are defaults. Future Edge or other store-assigned Chromium IDs are added with `conduit extension trust <id>`, which rewrites and verifies the user-level host manifests. Unknown callers are denied before the host reads a request.

Backend CI validates the backend distribution independently from the standalone extension repository. Store-ready browser archives are released by `conduit-extension`; the backend installer never downloads or unpacks them. This keeps browser-store review and backend security releases independently reproducible.
